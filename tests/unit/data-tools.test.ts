...
async function testDataTools() {
  ...
  console.log(`  [PASS] ACCT-001 retrieved: ${acc1.result.account_name}, Plan: ${acc1.result.plan}`);
  ...
  console.log(`  [PASS] Trace verified (duration: ${acc1.trace.durationMs}ms)`);
  ...
  console.log(`  [PASS] Retrieved ${orders.result.length} orders for ACCT-001`);
  ...
  console.log(`  [PASS] Specific order ORD-1001 retrieved: Status ${singleOrder.result[0].status}, Fee: ₹${singleOrder.result[0].shipment_fee_inr}`);
  ...
  console.log(`  [PASS] Retrieved ${tickets.result.length} tickets for ACCT-001`);
  ...
  console.log(`  [PASS] Specific ticket TKT-501 retrieved: "${singleTicket.result[0].subject}"`);
  ...
  try {
    ...
    if (err.name === 'ForbiddenError') {
      console.log('  [PASS] Customer ACCT-001 blocked from querying ACCT-002 account details.');
    } else {
      throw err;
    }
  }
  ...
  try {
    ...
    if (err.name === 'ForbiddenError') {
      console.log('  [PASS] Customer ACCT-001 blocked from querying ACCT-002 order (ORD-2001).');
    } else {
      throw err;
    }
  }
  ...
  try {
    ...
    if (err.name === 'ForbiddenError') {
      console.log('  [PASS] Customer ACCT-001 blocked from querying ACCT-002 ticket (TKT-502).');
    } else {
      throw err;
    }
  }
  ...
  if (intAcc1.result?.account_id === 'ACCT-001' && intAcc2.result?.account_id === 'ACCT-002') {
    console.log('  [PASS] Support agent successfully queried both ACCT-001 and ACCT-002.');
  } else {
  ...
  if (intOrd2001.result[0]?.order_id === 'ORD-2001') {
    console.log('  [PASS] Support agent successfully retrieved ORD-2001 (belonging to ACCT-002).');
  }
