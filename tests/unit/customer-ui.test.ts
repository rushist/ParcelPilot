...
async function testCustomerUI() {
  ...
  console.log('  [PASS] Northstar zero-fee cancellation verified.');
  ...
  console.log('  [PASS] LumenWorks credit policy verified.');
  ...
  console.log('  [PASS] Bulk upload 5,000-row limit and KI-208 workaround cited.');
  ...
  console.log('  [PASS] KI-211 status delay advisory cited.');
  ...
  console.log(`  [PASS] Proposed action created: ${propTurn.proposed_action.action_id} (${propTurn.proposed_action.title})`);

  const confResult = await confirmAction(customerNorthstar, propTurn.proposed_action.action_id);
  console.log(`  [PASS] Confirmed action via API: Status ${confResult.status}, Audit ID: ${confResult.audit_log_id}`);
  ...
  console.log('  [PASS] End-to-end confirmation and audit log verified.');
  ...
  console.log('  [PASS] Cross-account request strictly blocked at data layer.');
