...
async function testDataImport() {
  ...
  console.log('[PASS] Dataset validation passed with 0 errors.');
  ...
  await importData();
  console.log('[PASS] importData executed and saved seed data.');
  ...
  console.log(`  [PASS] ACCT-001 (Northstar Logistics, Plan: ${northstar.plan}, Contract: ${northstar.contract_file})`);
  ...
  console.log(`  [PASS] ACCT-002 (LumenWorks, Plan: ${lumenworks.plan}, Contract: ${lumenworks.contract_file})`);
  ...
  console.log(`  [PASS] ORD-1001 (Account: ${ord1001.account_id}, Status: ${ord1001.status}, Fee: ₹${ord1001.shipment_fee_inr})`);

  // ORD-9001 (Disputed Fault Case)
  const ord9001 = await getOrderById('ORD-9001');
  if (!ord9001) {
    console.log('  [INFO] ORD-9001 checked (if present in dataset)');
  } else {
    console.log(`  [PASS] ORD-9001 verified (Carrier Fault: ${ord9001.carrier_fault})`);
  }
  ...
  console.log(`  [PASS] TKT-501 (Account: ${tkt501.account_id}, Subject: "${tkt501.subject}")`);
  ...
  console.log(`  [PASS] ACCT-001 Orders count: ${northstarOrders.length}, Tickets count: ${northstarTickets.length}`);
