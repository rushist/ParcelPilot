...
4. **Observe Response & Sidecar Evaluation**:
   - **Deterministic Evaluation Sidecar**: Cancellation displays **`₹0 fee`** with subtitle `Signed Agreement Waiver`.
   - **Transcript**: Agent explains that while standard SOP v4 charges ₹250 for orders booked >30 minutes ago, Northstar's signed Enterprise Agreement Section 2 grants zero-fee cancellation for any BOOKED shipment prior to pickup.
...
4. **Observe Contract Evaluation**:
   - Agent identifies that pickup was delayed by 5 hours, applying LumenWorks custom contract terms (`INR 300` per 4-hour delay) rather than standard ₹150 SOP.
5. **Test Manager Approval Gate**:
   ...
   - Notice the card displays: **`⚠️ Requires Manager Approval (amount > INR 1,000)`**.
